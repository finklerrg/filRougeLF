...
public class SiteDeParisMetier {
   ...
   //   competiteurs=new LinkedList<Competiteur>();
      ...
      if(!leGestionnaire.getPasswordGestionnaire().equals(passwordGestionnaire)) throw new MetierException();
       ...
       int cont1=0;
        int var1=0;
            ...
            //primeiramente tem que ver qual a grana total
            long total=0;
            ...
            for(int i=0;i<tamanho2;i=i+1){
                if(competitions.get(var1).getCompetiteurs().get(i).getNom().equals(vainqueur)){//ve se a aposta esta certa
                    cont4=1;
                }
            }
            
            
            if(cont4==0) throw new CompetitionException(); //esse if ve se na linked list de competidores existe esse competidor
            ...
            long granacorreta=0;
            int cont3=0;
            System.out.println("vainqueur teste:"+competitions.get(0).getParis().get(0).getMiseEnJetons());
            System.out.println("entra no for");
            //System.out.println("vainqueur teste:"+competitions.get(var1).getParis().get(0).getCompetiteurReference().getNom());
            for(int j=0;j<competitions.get(var1).getParis().size();j++){
                //System.out.println("vainqueur teste:"+competitions.get(var1).getParis().get(j).getCompetiteurReference().getNom());
                if(competitions.get(var1).getParis().get(j).getCompetiteurReference().getNom().equals(vainqueur)){//ve se a aposta esta certa
                    //se aposta esta certa entao faz o seguinte
                    granacorreta=granacorreta+competitions.get(var1).getParis().get(j).getMiseEnJetons();
                    cont3=1;
                    
                }
            }
            
            //if(cont3==0) throw new CompetitionException();
            
            
                //agora credita os caras que acertaram com a grana de cada um 
             for(int i=0;i<tamanho;i++){
                if(competitions.get(var1).getParis().get(i).getCompetiteurReference().getNom().equals(vainqueur)){//ve se a aposta esta certa
                    //agora mexe na grana do cara debitando
                    ...
                    long crediter=(MiseEnJeton/granacorreta)*total;
                    
                    //crediterJoueur(String nom, String prenom, String pseudo, long sommeEnJetons, String passwordGestionnaire)
                    crediterJoueur(nom,prenom,pseudo,crediter,passwordGestionnaire) ;
	...
	public void debiterJoueur(String nom, String prenom, String pseudo, long sommeEnJetons, String passwordGestionnaire) throws  MetierException, JoueurInexistantException, JoueurException {
      ...
      if (joueurs.get(var2).getSommeEnJetons()<sommeEnJetons) throw new MetierException();
    ...
    public void miserVainqueur(String pseudo, String passwordJoueur, long miseEnJetons, String competition, String vainqueurEnvisage) throws MetierException, JoueurInexistantException, CompetitionInexistanteException, CompetitionException, JoueurException  {
      ...
      if (competitions.get(var3).getDateCloture().estDansLePasse()==true) throw new CompetitionException();            
        ...
        //Pari(long miseEnJetons, Joueur joueurReference, Competiteur competiteurReference)
        ...
        for(int i=0;i<joueurs.size();i=i+1){
         ...
         if(cont5==0) throw new JoueurInexistantException();
         
         
         if (joueurs.get(var5).getSommeEnJetons()<miseEnJetons) throw new MetierException();
         joueurs.get(var5).setSommeEnJetons(joueurs.get(var5).getSommeEnJetons()-miseEnJetons);
